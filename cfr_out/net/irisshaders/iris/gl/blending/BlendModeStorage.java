...
public class BlendModeStorage {
    ...
    public static void overrideBlend(BlendMode override) {
        if (!blendLocked) {
            GlStateManager.class_1017 blendState = GlStateManagerAccessor.getBLEND();
            originalBlendEnable = ((BooleanStateAccessor)blendState.field_5045).isEnabled();
            originalBlend = new BlendMode(blendState.field_5049, blendState.field_5048, blendState.field_5047, blendState.field_5046);
        }
        blendLocked = false;
        if (override == null) {
