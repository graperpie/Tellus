...
public class TextureTransformer {
        ...
        textureMap.forEach((stringTextureTypeTextureStageTri, s) -> {
            if (stringTextureTypeTextureStageTri.third() == stage) {
                ...
                for (Identifier id : root.identifierIndex.get(name)) {
                    ...
                    if (initDeclaration == null || (declaration = (DeclarationExternalDeclaration)initDeclaration.getAncestor(1, 0, DeclarationExternalDeclaration.class::isInstance)) == null || !((patt0$temp = initDeclaration.getType().getTypeSpecifier()) instanceof BuiltinFixedTypeSpecifier)) continue;
                    BuiltinFixedTypeSpecifier fixed = (BuiltinFixedTypeSpecifier)patt0$temp;
    ...
    private static boolean isTypeValid(TextureType expectedType, BuiltinFixedTypeSpecifier.BuiltinType extractedType) {
        return switch (expectedType) {
            ...
            case TextureType.TEXTURE_3D -> extractedType == BuiltinFixedTypeSpecifier.BuiltinType.SAMPLER3D || extractedType == BuiltinFixedTypeSpecifier.BuiltinType.ISAMPLER3D || extractedType == BuiltinFixedTypeSpecifier.BuiltinType.USAMPLER3D;
        };
    }
}
