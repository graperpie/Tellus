...
public class SodiumTransformer {
    ...
    public static void injectVertInit(ASTParser t, TranslationUnit tree, Root root, SodiumParameters parameters, boolean needsNormal) {
        tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"vec3 _vert_position;", "vec2 _vert_tex_diffuse_coord;", "vec2 _vert_tex_diffuse_coord_bias;", "vec2 _vert_tex_light_coord;", "vec4 _vert_color;", "const uint POSITION_BITS        = 20u;", "const uint POSITION_MAX_COORD   = 1u << POSITION_BITS;", "const uint POSITION_MAX_VALUE   = POSITION_MAX_COORD - 1u;", "const uint TEXTURE_BITS         = 15u;", "const uint TEXTURE_MAX_COORD    = 1u << TEXTURE_BITS;", "const uint TEXTURE_MAX_VALUE    = TEXTURE_MAX_COORD - 1u;", "const float VERTEX_SCALE = 32.0 / POSITION_MAX_COORD;", "const float VERTEX_OFFSET = -8.0;", "uint _draw_id;", "vec3 irs_Normal;", "vec4 irs_Tangent;", "const uint MATERIAL_USE_MIP_OFFSET = 0u;", "vec3 oct_to_vec3(vec2 e) {\n\tvec2 f = vec2(e.x * 2.0f - 1.0f, e.y * 2.0f - 1.0f);\n\tvec3 n = vec3(f.x, f.y, 1.0f - abs(f.x) - abs(f.y));\n\tfloat t = clamp(-n.z, 0.0f, 1.0f);\n\tn.x += n.x >= 0.0f ? -t : t;\n\tn.y += n.y >= 0.0f ? -t : t;\n\treturn normalize(n);\n}\n", "vec4 tangent_decode(vec2 e) {\n\tvec2 oct_compressed = e;\n\toct_compressed.y = oct_compressed.y * 2 - 1;\n\tfloat r_sign = oct_compressed.y >= 0.0f ? 1.0f : -1.0f;\n\toct_compressed.y = abs(oct_compressed.y);\n\tvec3 res = oct_to_vec3(oct_compressed.xy);\n\treturn vec4(res, r_sign);\n}\n", "\tuvec3 _deinterleave_u20x3(uvec2 data) {\n\t     uvec3 hi = (uvec3(data.x) >> uvec3(0u, 10u, 20u)) & 0x3FFu;\n\t     uvec3 lo = (uvec3(data.y) >> uvec3(0u, 10u, 20u)) & 0x3FFu;\n\n\t     return (hi << 10u) | lo;\n\t }\n\t", "\tvec2 _get_texcoord() {\n\t     return vec2(a_TexCoord & TEXTURE_MAX_VALUE) / float(TEXTURE_MAX_COORD);\n\t }\n", "\tvec2 _get_texcoord_bias() {\n\t\t\t\t\t\treturn mix(vec2(-1.0), vec2(1.0), bvec2(a_TexCoord >> TEXTURE_BITS));\n\t\t\t\t}\n", "float _material_mip_bias(uint material) {\n    return ((material >> MATERIAL_USE_MIP_OFFSET) & 1u) != 0u ? 0.0f : -4.0f;\n}", "void _vert_init() {_vert_position = ((_deinterleave_u20x3(a_Position) * VERTEX_SCALE) + VERTEX_OFFSET);_vert_tex_diffuse_coord = _get_texcoord();_vert_tex_diffuse_coord_bias = _get_texcoord_bias();_vert_tex_light_coord = vec2(a_LightAndData.xy);_vert_color = a_Color;" + (needsNormal ? "irs_Normal = oct_to_vec3(iris_Normal.xy);" : "") + (needsNormal ? "irs_Tangent = tangent_decode(iris_Normal.zw);" : "") + "_draw_id = a_LightAndData[3]; }", "uvec3 _get_relative_chunk_coord(uint pos) {\n    // Packing scheme is defined by LocalSectionIndex\n    return uvec3(pos) >> uvec3(5u, 0u, 2u) & uvec3(7u, 3u, 7u);\n}", "vec3 _get_draw_translation(uint pos) {\n    return _get_relative_chunk_coord(pos) * vec3(16.0f);\n}\n"});
        CommonTransformer.addIfNotExists(root, t, tree, "a_Position", Type.U32VEC2, StorageQualifier.StorageType.IN);
        CommonTransformer.addIfNotExists(root, t, tree, "a_TexCoord", Type.U32VEC2, StorageQualifier.StorageType.IN);
        CommonTransformer.addIfNotExists(root, t, tree, "a_Color", Type.F32VEC4, StorageQualifier.StorageType.IN);
        CommonTransformer.addIfNotExists(root, t, tree, "a_LightAndData", Type.U32VEC4, StorageQualifier.StorageType.IN);
        if (needsNormal) {
            CommonTransformer.addIfNotExists(root, t, tree, "iris_Normal", Type.F32VEC4, StorageQualifier.StorageType.IN);
        }
