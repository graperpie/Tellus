...
implements IrisTextVertexSink {
    static final class_293 format = IrisVertexFormats.GLYPH;
    ...
    public IrisTextVertexSinkImpl(int maxQuadCount, IntFunction<ByteBuffer> buffer) {
        this.buffer = buffer.apply(format.method_1362() * 4 * maxQuadCount);
    ...
    private void vertex(float x, float y, float z, int color, float u, float v, int light) {
        ...
        if (this.vertexCount == 4) {
            ...
            this.uSum *= 0.25f;
            this.vSum *= 0.25f;
            this.quad.setup(this.elementOffset, IrisVertexFormats.GLYPH.method_1362());
