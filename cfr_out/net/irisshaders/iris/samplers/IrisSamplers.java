...
public class IrisSamplers {
    ...
    public static boolean addShadowSamplers(SamplerHolder samplers, ShadowRenderTargets shadowRenderTargets, ImmutableSet<Integer> flipped, boolean separateHardwareSamplers) {
        ...
        if (waterShadowEnabled) {
            ...
            samplers.addDynamicSampler(TextureType.TEXTURE_2D, shadowRenderTargets.getDepthTextureNoTranslucents()::getTextureId, separateHardwareSamplers ? null : shadowRenderTargets.getSamplerFor(1), "shadowtex1", "shadow");
        } else {
    ...
    public static void addLevelSamplers(SamplerHolder samplers, WorldRenderingPipeline pipeline, class_1044 whitePixel, boolean hasTexture, boolean hasLightmap, boolean hasOverlay) {
        if (hasTexture) {
            samplers.addExternalSampler(0, "tex", "texture", "gtexture");
        } else {
            samplers.addDynamicSampler(() -> ((class_1044)whitePixel).method_4624(), "tex", "texture", "gtexture", "gcolor", "colortex0");
        }
        if (hasLightmap) {
            samplers.addExternalSampler(2, "lightmap");
        } else {
            samplers.addDynamicSampler(() -> ((class_1044)whitePixel).method_4624(), "lightmap");
        }
        if (hasOverlay) {
            samplers.addExternalSampler(1, "iris_overlay");
        } else {
            samplers.addDynamicSampler(() -> ((class_1044)whitePixel).method_4624(), "iris_overlay");
        }
